...
(function() {
            ...
            if (config.buffer) {
              ...
              if (config.format === 'json') {
                try {
                  ...
                  complete(0, 1);
                  if (!isError) {
                    return callback("JSON对象转换失败：" + e + "，内容是" + content, content, res);
                  }
                }
              } else {
                if (config.format === 'jsonp' && config.data && config.data.callback) {
                  try {
                    ...
                    complete(0, 1);
                    if (!isError) {
                      return callback("JSONP对象转换失败：" + e + "，内容是" + content, content, res);
                    }
                  }
                } else {
                  complete(1, 0);
  ...
  httpUtil.download = function(source, target, callback) {
    var dir;
